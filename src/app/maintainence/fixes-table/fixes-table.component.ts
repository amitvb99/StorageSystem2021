...
export class FixesTableComponent implements OnInit {
  ...
  fixes_meta_data  = {
    ...
    columns_count:9,
    columns:['instrument', 'maintainer_name', 'maintainer_phone', 'maintainer_address', 'openning_user', 'closing_user', 'notes', 'from', 'status', 'to'],
    headers:{
  ...
  functions={
    ...
    'end_fix': (data_to_show,i) => {
      var fix_id = i._id
      this.crud.end_loan(fix_id).subscribe(res => {
        if (res !== null){
